...
import { deleteProjectFromGitHub } from '../../../../lib/github';
...
export default async function handler(req, res) {
  ...
  const rl = rateLimit(`delete-project:${ip}`, 20, 60 * 60 * 1000);
  ...
  try {
    await deleteProjectFromGitHub(slug, sha);
    console.info(`[ADMIN DELETE PROJECT] "${slug}" at ${new Date().toISOString()}`);
    return res.status(200).json({ success: true, message: `Project "${slug}" deleted.` });
  } catch (err) {
    if (err.status === 404) return res.status(404).json({ error: 'Project not found on GitHub.' });
    if (err.status === 409) return res.status(409).json({ error: 'File was modified — refresh and try again.' });
    return res.status(500).json({ error: 'Failed to delete project from GitHub.' });
  }
}
