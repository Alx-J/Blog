...
tags:
  ...
  - windows
category: Write-up
description: "Administrator is a medium Windows box centered on full domain compromise via ACL abuse and credential chaining: enumerate ACLs to find Olivia’s GenericAll over Michael and reset his password; as Michael force a reset for Benjamin to gain FTP access and crack backup.psafe3 for creds; spray those creds to find Emily; use Emily’s GenericWrite over Ethan to perform a targeted Kerberoast, crack Ethan’s hash, then use his DCSync rights to dump AD and recover the Administrator hash."
---
...
### Win-rm
By using evil-winrm we got a Powershell
...
→ It shows a way to user `michael` which is `Generic All` ⇒ permission to change password to user
...
**In Bloodhound:**
→ now we check **michael** it has also FDOC now , when we click on it 
  → It shows a way to another user who is `Benjamin` where we can perform `force change password`
...
```
  → To open we need  a **master password** it seems
  
```sh
hashcat -m 5200 Backup.psafe3 /rockyou.txt
```
->  It will give us master password from `backup.psafe3`. 
...
## Impackets
We seen that ethan FDOC is DCSync then we used `secretsdump.py` from `/impacket/example`
